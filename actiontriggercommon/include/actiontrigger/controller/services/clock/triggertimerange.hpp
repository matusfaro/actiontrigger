...
#ifndef TriggerTimeRange_HPP_
#define TriggerTimeRange_HPP_
...
namespace actiontrigger
{
    ...
    class TriggerTimeRange: public Trigger
    {
    public:
        TriggerTimeRange(StatementModel* model);
        ~TriggerTimeRange();
    ...
    private:
        bool isActive(QTime startTime, QTime duration, QDateTime now, ExecutionState* state);
        void blockUntilActive(QTime startTime, QDateTime now, ExecutionState* state);
        int getTotalSeconds(QTime time);
        static Logger* StatementLOG;
        static std::vector<ParameterDefinition*>* parameterDefinitions;
    };

} /* namespace actiontrigger */
#endif /* TriggerTimeRange_HPP_ */
